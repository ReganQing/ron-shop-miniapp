...
onReady(() => {
  ...
  // 动画效果，导航栏返回按钮
  currentPageInstance.animate('.navbar .back', [{ color: '#fff' }, { color: '#000' }], 1000, {
    scrollSource: '#scroller',
    timeRange: 1000,
    startScrollOffset: 0,
    endScrollOffset: 50,
...
const getMemberOrderByIdData = async () => {
  ...
  if (
    [OrderState.ALREADYDONE, OrderState.WAITTOCOMMIT, OrderState.WAITTOACCEPT].includes(
...
const OnTimeUp = () => {
  // 修改订单状态
  orderDetail.value!.orderState = OrderState.ALREADYCANCEL
}
...
const onOrderPay = async () => {
  if (isDEV) {
    ...
    // 生产环境正式微信支付
    const res = await payWxPayMiniPayAPI({ orderId: query.id })
    ...
    <template v-if="orderDetail">
      ...
      <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        ...
        <template v-else>
          ...
          <!-- 待收货状态: 展示确认收货 -->
          <view class="button primary" v-if="orderDetail.orderState === OrderState.WAITTOACCEPT">
            确认收货
          </view>
          <!-- 待评价状态: 展示去评价 -->
          <view class="button" v-if="orderDetail.orderState === OrderState.WAITTOCOMMIT">
            去评价
          </view>
          ...
          <view
            class="button delete"
            v-if="
              [OrderState.ALREADYDONE, OrderState.WAITTOCOMMIT, OrderState.ALREADYCANCEL].includes(
